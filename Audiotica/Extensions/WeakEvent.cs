...
namespace Audiotica.Extensions
{
    /// <summary>
        ...
        private void Subscribe(object source, string eventName)
        {
            ...
            if (_eventInfo == null)
            {
                throw new InvalidOperationException(
                    $"RuntimeEvent '{eventName}' was not found on type '{source.GetType().Name}'");
            }
    ...
    public class WeakEvent : EventWatcher
    {
        ...
        private static WeakEvent FindRegistration(object source, string eventName)
        {
            ...
            for (var iReg = Registrations.Count - 1; iReg >= 0; iReg--)
            {
                ...
                if ((reg.source.Target == source) && (reg._eventName == eventName))
                {
                    break;
                }
                reg = null;
        ...
        private static void VerifyDelegate<THandler>() where THandler : class
        {
            ...
            {
                throw new InvalidOperationException(
                    $"The type '{typeof (THandler).Name}' is not a delegate and cannot be used for event subscriptions.");
            }
        }
